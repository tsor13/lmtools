...
class Postprocessor:
    def __init__(self, results_fname, save_fname, matching_strategy=None):
        ...
        print(
            f"Dropping {num_missing} instances with missing responses from",
            results_fname,
        )
    ...
    def calculate_correct_weight(self, df):
        ...
        get_correct_weight = lambda row: max(
            row["probs"].get(row["ground_truth"], 0),
            row["probs"].get(row["ground_truth"].lower(), 0),
        )
...
def main():
    ...
    parser.add_argument(
        "-f", "--filename", type=str, help="path to file to process", required=True
    )
    parser.add_argument("-s", "--save", type=str, help="path to save processed file to")
